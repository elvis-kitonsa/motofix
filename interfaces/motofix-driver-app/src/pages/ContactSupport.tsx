// ContactSupport.tsx — the help/support screen: ways to reach the MOTOFIX team (phone,
// WhatsApp, email) and common questions, for when a driver needs a human.
...
const TOPICS = [
  { id: 'active_request', label: 'Active request', Icon: Wrench, accent: R },
  { id: 'location', label: 'Location issue', Icon: MapPin, accent: '#60A5FA' },
  { id: 'payment', label: 'Quote or payment', Icon: Receipt, accent: Y },
  { id: 'account', label: 'Account access', Icon: Smartphone, accent: '#A78BFA' },
...
export default function ContactSupport() {
  ...
  const [topic, setTopic] = useState('active_request');
  ...
  const submit = () => {
    ...
    localStorage.setItem('motofix_support_draft', JSON.stringify({
      topic,
        ...
        <section style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              borderRadius: 18,
              background: `${G}18`,
              border: `1.5px solid ${G}3d`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              marginBottom: 16,
              boxShadow: `0 0 28px ${G}16`,
            }}>
              {sent
            ...
            <p style={{ color: 'var(--text-md)', fontSize: 13, lineHeight: 1.65, maxWidth: 580 }}>
              {sent
                ...
                : 'Reach the MOTOFIX team for active request issues, location problems, payments, account access, safety concerns, or general feedback.'}
            </p>
          </div>
          ...
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 10, marginBottom: 20 }}>
              <ActionButton
                ...
                sub={SUPPORT_EMAIL}
                accent={G}
                onClick={() => { window.location.href = `mailto:${SUPPORT_EMAIL}?subject=MOTOFIX%20Driver%20Support`; }}
              />
              <ActionButton
                icon={Phone}
                label="Call support"
              ...
              <ActionButton
                ...
                accent="#22C55E"
                onClick={() => { window.location.href = `https://wa.me/${WHATSAPP_PHONE.replace('+', '')}?text=Hello%20MOTOFIX%20support`; }}
              />
            </div>
