...
const QUICK_REPLIES = [
  ...
  "I'm waiting at my car 🚗",
  "How long will it take? ⏱️",
  "Please hurry, it's urgent ⚠️",
]
...
function mechInitials(name?: string | null) {
  if (!name) return 'M'
...
function toUI(m: JobChatMessage): UIMsg {
  return {
    ...
    seen: m.seen_by_mechanic,   // our (driver's) message has been read by the mechanic
  }
}
...
export default function DriverChat() {
  ...
  const request = requests.find(r => String(r.id) === requestId)
  const mechanicName = (request as any)?.mechanic_name ?? null
  ...
  const sendTyping = useCallback((typing: boolean) => {
    if (!requestId) return
    sendMessage({ type: 'chat_typing', service_request_id: requestId, role: 'driver', typing })
  }, [requestId, sendMessage])

  const sendSeen = useCallback(() => {
    if (!requestId) return
    sendMessage({ type: 'chat_seen', service_request_id: requestId, seen_by: 'driver' })
  }, [requestId, sendMessage])
  ...
  // Load history (and mark the mechanic's messages as seen)
  useEffect(() => {
    if (!requestId) return
    let cancelled = false
    chatService.list(requestId, 'driver', true)
      .then(res => {
        if (cancelled) return
        const list = (res.data?.messages ?? []).map(toUI)
        list.forEach(m => seenIds.current.add(m.id))
        setMessages(list)
        sendSeen()  // let the mechanic know we've opened & read the thread
      })
      .catch(() => {})
  ...
  // Live WebSocket events: new messages, the mechanic's typing state, and
  // read-receipts telling us the mechanic has seen our messages.
  useEffect(() => {
    ...
    if (msg.type === 'chat_message') {
      ...
      addMsg({
        ...
        isOwn: msg.sender_role === 'driver',
        createdAt: msg.created_at ?? new Date().toISOString(),
      })
      if (msg.sender_role === 'mechanic') {
        // We're looking at the thread → mark seen + tell the mechanic we read it
        chatService.markSeen(requestId!, 'driver').catch(() => {})
        sendSeen()
      }
    } else if (msg.type === 'chat_typing' && msg.role === 'mechanic') {
      if (msg.typing) {
        setOtherTyping(true)
    ...
    } else if (msg.type === 'chat_seen' && msg.seen_by === 'mechanic') {
  ...
  const sendText = async (text?: string) => {
    ...
    try {
      const res = await chatService.sendText(requestId, { sender_role: 'driver', sender_id: myId, body })
      ...
      setInputText(body) // restore so the user can retry
    }
  }
  ...
  const sendMedia = async () => {
    ...
    form.append('sender_role', 'driver')
  ...
  const onSendPress = () => {
    if (audioBlob || attachment) sendMedia()
    else sendText()
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--page-bg)', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <div style={{ width: 42, height: 42, borderRadius: '50%', background: 'linear-gradient(135deg, #F59E0B, #D97706)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800, color: '#000' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {showQuickReplies && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '30px 0 16px', gap: 14 }}>
            <div style={{ width: 56, height: 56, borderRadius: 18, background: 'rgba(245,158,11,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26 }}>💬</div>
            <p style={{ fontSize: 13.5, color: 'var(--text-md)', fontWeight: 600, textAlign: 'center', maxWidth: 240, lineHeight: 1.5 }}>
              Send {mechanicName?.split(' ')[0] ?? 'your mechanic'} a message — coordinate your meeting point.
            </p>
          ...
          return (
            <div key={msg.id} style={{ animation: 'msg-in 0.18s ease both' }}>
              ...
              <div style={{ display: 'flex', justifyContent: msg.isOwn ? 'flex-end' : 'flex-start', marginBottom: prevOwn ? 2 : 8 }}>
                {!msg.isOwn && !prevOwn && (
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#F59E0B', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#000', flexShrink: 0, marginRight: 8, alignSelf: 'flex-end', marginBottom: 4 }}>
                    {mechInitials(mechanicName)}
                  </div>
                )}
        ...
        {/* WhatsApp-style "Seen" under the last sent message once the mechanic reads it */}
        {(() => {
        ...
        {otherTyping && (
          <div style={{ display: 'flex', justifyContent: 'flex-start', marginBottom: 8, animation: 'msg-in 0.18s ease both' }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#F59E0B', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#000', flexShrink: 0, marginRight: 8, alignSelf: 'flex-end' }}>
              {mechInitials(mechanicName)}
            </div>
