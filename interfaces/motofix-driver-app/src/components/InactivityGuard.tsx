...
export default function InactivityGuard() {
  ...
  const goAhead = () => {
    clearInactivity()
    setShow(false)
    navigate('/', { replace: true }) // splash → welcome → driver-entry → login
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 380, background: 'var(--surface-1, #14141f)', borderRadius: 22, border: '1px solid var(--border-3, rgba(255,255,255,0.12))', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', padding: '28px 24px', textAlign: 'center' }}>
